...
app.use((error, request, response, next)=> {
  if(error.code == "EBADCSRFTOKEN"){
    response.status(403).send("The token is invalid.");
  } else {
    next();
  }
})
...
app.post("/auth", function (request, response) {
  ...
  if (username && password) {
    db.get(
      // Changed template literals to prepared statement
...
app.post("/download", function (request, response) {
  if (request.session.loggedin) {
    ...
    const rootDirectory = "historyFiles\\";
    const filePath = path.join(process.cwd(), "/history_files/", file_name);
    const filename = path.content( filePath );
...
app.post("/public_forum", function (request, response) {
  if (request.session.loggedin) {
    // Added .blacklist method for certain chars
    // var comment = query(request.body.comment).blacklist("-;'<>");
    var comment = query(request.body.comment).escape();
    
    var username = request.session.username;
    if (comment) {
      db.all(
        // Changed template literals to placeholder values
